...
合约 1：卡池代理
合约 2：卡池实现，继承了 VRF consumer。主要的逻辑
合约 3：卡池信标
合约 4：随机数 VRF Coordinator Mock
合约 5：Gacha card NFT
...
实际合约中的变量顺序经过 gas 优化和 size 优化，不应随意调整顺序。
...
- 随机数请求(RandomResult): 稀有度结果，随机数个数，随机数数组，
- 卡池设置(PoolConfig)
- 合约命名空间存储(ERC 7201)
...
- CALLBACK_GAS_LIMIT: 返回随机数 tx 的 gas 限制。这个非常关键，见代码注释
...
- guarantee: 是否十连保底机制
- discountGachaTen: 十连费用折扣
- claimSigner: claim 签名者，应该使用单独的一个地址
- guaranteeRarity: 保底稀有度
- PROCESSING_CAP: 未结算的请求数量限制
...
1. 部署合约。创建卡池，编号，总量，概率，单次费用。由卡池创建 NFT 合约（必须）。
...
7. 合约验证签名后，更新对应 reqId 的信息。由 Pool 合约调用 NFT 合约，指定稀有度 mint。
...
由于向合约写入的随机数任何人都能看到，所以 mock 版本只能用于测试环境。（生产环境的 VRF 并非如此）
...
3. 外部（生产环境预言机，开发环境 feed）给出随机数，并回调 consumer （就是卡池） 的 `rawFulfillRandomWords` 方法。
4. `rawFulfillRandomWords` 会调用卡池的 `fulfillRandomWords`。 真正地将随机数记录下来，处理业务逻辑。
5. VRF feed 的实现是监听事件并调用 mock 版本 VRF 的 `fulfillRandomWordsWithOverride` 方法。然后像真正的 VRF 一样调用 consumer 的 `rawFulfillRandomWords`。 只不过随机数来自于脚本的投喂。
